...
def edit():
    ...
    if request.method == 'POST':
        ...
        cursor = db_conn.cursor()
        update_sql = 'UPDATE student SET StudentName = %s, StudentGender = %s, StudentProgramme = %s, StudentState = %s,StudentPhoneNumber = %s,StudentYear = %s,StudyMethod = %s,ResumeLink = %s,ReportLink = %s WHERE StudentID = 1'
...
def appliedStudent(type=None):
    ...
    # Fetch student data from the database where StudentApplyStatus is 'Pending'
    cursor = db_conn.cursor()
    select_sql = "SELECT A.ApplicationID, S.StudentName, S.StudentProgramme, J.JobTitle, A.ApplicationStatus, S.ResumeLink FROM application A JOIN student S ON A.StudentID = S.StudentID JOIN jobs J ON A.JobID = J.JobID WHERE A.ApplicationStatus = '"+type+"'"
    cursor.execute(select_sql)
...
def markingReport():
    ...
    cursor = db_conn.cursor()
    cursor.execute("SELECT StudentID, StudentName, StudentProgramme, StudentYear, StudyMethod, ReportMark, ReportDate,ReportLink FROM student WHERE ReportLink IS NOT NULL")
